...
<body>
        ...
        <div class="Titulo">
          Resultados de las Votaciones de Egresados [Periodo: {{$periodoActivo[0]->periodo}}]
        </div> 
        ...
        <div class="Tittle-Table">
          ...
          <table class="table Tablas"> 
            ...
            <tbody>
              @foreach($egresados_candidatos as $egre)
              <?php
                $egresado = \DB::table('users')
                ->where('cedula','=',$egre->cedula)->get();
              ?>
              <tr>
              <th>{{$egresado[0]->nombres.' '.$egresado[0]->apellidos}}</th>
              <td class="font-weight-bold">{{$egre->votos}}</td> 
              </tr>
              @endforeach
            </tbody>
          </table>
