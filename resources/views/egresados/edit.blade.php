        ...
        <!-- Form Element sizes -->
          <div class="box box-success">
                <div class="box-header with-border">
                  <h3 class="box-title">Egresados</h3>
                </div>
                <div class="box-body">
                <form method="POST" action="{{action('EgresadosController@update',$egresados[0]->cedula)}}" nctype="multipart/form-data">
                @csrf
                {{ method_field('PATCH') }}
                <label>Cedula</label>
                  <input class="form-control" type="text" placeholder="Cedula" name="cedula" value='{{$egresados[0]->cedula}}' disabled>
                  <br>
                <label>Nombre</label>
                  ...
                  <select name="code_escuela" class="form-control">
                      @foreach ($escuelas as $item)
                        @if($item->code_escuela == $egresados[0]->code_escuela)
                  ...
                  <br>
                  <label>Foto</label>
                  <input class="form-control" type="text" placeholder="Foto" name="foto" value='{{$egresados[0]->foto}}'>
                  <br>
                  <label>Ubicacion</label>
                  <input class="form-control" type="text" placeholder="Ubicacion" name="ubicacion" value='{{$egresados[0]->ubicacion}}'>
                  <br>
                  <label>Fecha de egreso</label>
                  <input class="form-control" type="date" placeholder="Fecha Egreso" name="fecha_egreso" value='{{$egresados[0]->fecha_egreso}}'>
                  <br>
